...
public class PotentialTheftRatingRelInstances {
	...
	static {
		...
		// rule annotations for rule AE_PTC01: _car.getConvertible() ->
		// PotentialTheftRating(_car,"high");
		_annotations_AE_PTC01.put("description", "If the car is a convertible, then the car's potential theft rating is high");

		// relationship annotations for rule AE_PTC02: (_car.getPrice())>45000
		// -> PotentialTheftRating(_car,"high");
		...
		// rule annotations for rule AE_PTC02: (_car.getPrice())>45000 ->
		// PotentialTheftRating(_car,"high");
		...
		// relationship annotations for rule AE_PTC03:
		// highTheftProbabilityAutoList.contains(_car.getType()) ->
		// PotentialTheftRating(_car,"high");
		...
		// rule annotations for rule AE_PTC03:
		// highTheftProbabilityAutoList.contains(_car.getType()) ->
		// PotentialTheftRating(_car,"high");
		...
		// relationship annotations for rule AE_PTC04: (_car.getPrice())>20000 &
		// (_car.getPrice())<45000 &
		// !(highTheftProbabilityAutoList.contains(_car.getType())) ->
		// PotentialTheftRating(_car,"moderate");
		...
		// rule annotations for rule AE_PTC04: (_car.getPrice())>20000 &
		// (_car.getPrice())<45000 &
		// !(highTheftProbabilityAutoList.contains(_car.getType())) ->
		// PotentialTheftRating(_car,"moderate");
		_annotations_AE_PTC04
				.put("description",
						"If all of the following are true, then the car's potential theft rating is moderate: car's price is between $20,000 and $45,000, car model is not on the list of 'High Theft Probability Auto'");

		// relationship annotations for rule AE_PTC05: (_car.getPrice())<20000 &
		// !(highTheftProbabilityAutoList.contains(_car.getType())) ->
		// PotentialTheftRating(_car,"low");
		...
		// rule annotations for rule AE_PTC05: (_car.getPrice())<20000 &
		// !(highTheftProbabilityAutoList.contains(_car.getType())) ->
		// PotentialTheftRating(_car,"low");
		_annotations_AE_PTC05.put("description",
	...
	private static ResourceIterator<PotentialTheftRatingRel> getRating_1(final DerivationController _derivation, final Car car) {
		...
		// case 4
		if (!((_bindings._car.getPrice()) > 45000)) {
			return EmptyIterator.DEFAULT;
		}
	...
	private static ResourceIterator<PotentialTheftRatingRel> getRating_2(final DerivationController _derivation, final Car car) {
		...
		// case 4
		if (!(_bindings.highTheftProbabilityAutoList.contains(_bindings._car.getType()))) {
			return EmptyIterator.DEFAULT;
		}
	...
	private static ResourceIterator<PotentialTheftRatingRel> getRating_3(final DerivationController _derivation, final Car car) {
		...
		// !(highTheftProbabilityAutoList.contains(_car.getType())) ->
		// PotentialTheftRating(_car,"moderate");
		// prereqs: [(_car.getPrice())>20000, (_car.getPrice())<45000,
		// !(highTheftProbabilityAutoList.contains(_car.getType()))]
		class _Bindings {
			private org.mandarax.examples.userv.domain.Car _car = car;
			private java.util.Collection highTheftProbabilityAutoList = PotentialTheftRatingRelInstances.highTheftProbabilityAutoList;
		...
		// case 4
		if (!((_bindings._car.getPrice()) > 20000)) {
			return EmptyIterator.DEFAULT;
		}
