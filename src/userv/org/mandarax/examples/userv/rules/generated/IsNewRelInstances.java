...
/**
 * Interface for queries for relationship <strong>IsNew</strong>. Generated by
...
public class IsNewRelInstances {
	...
	static {
		// relationship annotations for rule AP_05:
		// (_car.getModelYear())==CurrentYear -> IsNew(_car);
		...
		// rule annotations for rule AP_05: (_car.getModelYear())==CurrentYear
		// -> IsNew(_car);
		_annotations_AP_05.put("description", "If the model year is the same as the current year, then new car");

		// relationship annotations for rule AP_06:
		// (_car.getModelYear())==NextYear -> IsNew(_car);
		...
		// rule annotations for rule AP_06: (_car.getModelYear())==NextYear ->
		// IsNew(_car);
	...
	static ResourceIterator<IsNewRel> isNew(final DerivationController _derivation, final Car car) {
		...
		return new IteratorChain<IsNewRel>(2) {
			public ResourceIterator<IsNewRel> getNextIterator(int pos) {
				switch (pos) {
				case 0:// invoke AP_05: (_car.getModelYear())==CurrentYear ->
						// IsNew(_car);
					return isNew_0(_derivation.pop(_derivationlevel), car);

				case 1:// invoke AP_06: (_car.getModelYear())==NextYear ->
						// IsNew(_car);
	...
	private static ResourceIterator<IsNewRel> isNew_0(final DerivationController _derivation, final Car car) {
		_derivation.log("IsNew.AP_05", DerivationController.RULE, _annotations_AP_05);
		...
		// case 4
		if (!(org.mandarax.rt.Equals.compare(_bindings._car.getModelYear(), _bindings.CurrentYear))) {
			return EmptyIterator.DEFAULT;
		}
	...
	private static ResourceIterator<IsNewRel> isNew_1(final DerivationController _derivation, final Car car) {
		_derivation.log("IsNew.AP_06", DerivationController.RULE, _annotations_AP_06);
		...
		// case 4
		if (!(org.mandarax.rt.Equals.compare(_bindings._car.getModelYear(), _bindings.NextYear))) {
			return EmptyIterator.DEFAULT;
		}
