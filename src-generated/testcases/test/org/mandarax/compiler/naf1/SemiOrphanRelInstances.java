...
public class SemiOrphanRelInstances {
	...
	static {
		// relationship annotations for rule rule1: isYoung(p) & Father(f,p) &
		// not Mother(m,p) -> SemiOrphan(p);

		// rule annotations for rule rule1: isYoung(p) & Father(f,p) & not
		// Mother(m,p) -> SemiOrphan(p);

		// relationship annotations for rule rule2: isYoung(p) & not Father(f,p)
		// & Mother(m,p) -> SemiOrphan(p);

		// rule annotations for rule rule2: isYoung(p) & not Father(f,p) &
		// Mother(m,p) -> SemiOrphan(p);
