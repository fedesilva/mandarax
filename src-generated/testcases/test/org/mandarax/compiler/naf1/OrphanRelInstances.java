...
public class OrphanRelInstances {
	...
	static {
		// relationship annotations for rule  rule1: isYoung(p) & not Father(f,p) & not Mother(m,p) -> Orphan(p);
		
		// rule annotations for rule  rule1: isYoung(p) & not Father(f,p) & not Mother(m,p) -> Orphan(p);
	...
	static ResourceIterator<OrphanRel> isOrphan ( final DerivationController _derivation ,  final Person person  ) {
		...
		return new IteratorChain<OrphanRel>(1) {
			...
			switch (pos) {
                		...
                		case 0: {
                			// invoke rule1: isYoung(p) & not Father(f,p) & not Mother(m,p) -> Orphan(p);
