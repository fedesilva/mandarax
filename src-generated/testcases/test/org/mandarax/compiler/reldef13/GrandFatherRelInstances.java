...
public class GrandFatherRelInstances {
	...
	// rule: rule1: (x.getName())=="Otto" & Father(x,y) & Father(y,z) ->
	// GrandFather(x.getName(),z.getName());
	private final static java.util.Properties _annotations_rule1 = new java.util.Properties();

	// initialise annotations
	static {
		// relationship annotations for rule rule1: (x.getName())=="Otto" &
		// Father(x,y) & Father(y,z) -> GrandFather(x.getName(),z.getName());

		// rule annotations for rule rule1: (x.getName())=="Otto" & Father(x,y)
		// & Father(y,z) -> GrandFather(x.getName(),z.getName());
	...
	static ResourceIterator<GrandFatherRel> getGrandChildren(final DerivationController _derivation, final String grandFather) {
		...
		return new IteratorChain<GrandFatherRel>(1) {
			...
			public ResourceIterator<GrandFatherRel> getNextIterator(int pos) {
				...
				case 0: {
					// invoke rule1: (x.getName())=="Otto" & Father(x,y) &
					// Father(y,z) -> GrandFather(x.getName(),z.getName());
	...
	private static ResourceIterator<GrandFatherRel> getGrandChildren_0(final DerivationController _derivation, final String grandFather) {
		...
		// rule: rule1: __t0=="Otto" & Father(x,y) & Father(y,z) &
		// __t0==(x.getName()) -> GrandFather(__t0,z.getName());
		// prereqs: [__t0=="Otto", Father(x,y), __t0==(x.getName()),
		// Father(y,z)]
		class _Bindings {
		...
		if (!(org.mandarax.rt.Equals.compare(_bindings.__t0, "Otto"))) {
			return EmptyIterator.DEFAULT;
		}
		...
		final ResourceIterator<FatherRel> iterator3 = new FilteredIterator<FatherRel>(iterator2, _filter3);

		// apply prerequisite Father(y,z)
		final ResourceIterator<FatherRel> iterator4 = new NestedIterator<FatherRel, FatherRel>(iterator3) {
		...
		// rule head
		return new NestedIterator<FatherRel, GrandFatherRel>(iterator4) {
